...
import { clearInterceptIntent } from '@/services/authGuard';
import { resumeInterruptedPageAfterLogin } from '@/services/login';
import { toFollowRecommendations, toWelcomeTour } from '@/routers/user';
...
describe('dialect onboarding page', () => {
  ...
  it('loads a real sample and sends new users to the welcome tour after saving', async () => {
    const wrapper = mountPage();
    ...
    expect(clearInterceptIntent).toHaveBeenCalled();
    expect(toWelcomeTour).toHaveBeenCalledWith(true);
  ...
  it('keeps the resume flow for existing users picking a missing dialect', async () => {
    ...
    expect(toWelcomeTour).not.toHaveBeenCalled();
  });
